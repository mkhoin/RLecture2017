...
```{r}
id=1:10
eng=c(70,NA,80,85,90,90,60,55,NA,90)
math=c(80,73,84,90,95,85,60,50,NA,100)
data=data.frame(id,eng,math)
data
```
...
longdf=melt(data,id="id",variable.name="subject",value.name="score")
#longdf
result=ddply(longdf,.(subject),summarize,결시자=sum(is.na(score)),평균=mean(score,na.rm=T))
result
```
...
1. 데이타 프레임 전체를 생각하지 말고 하나의 벡터에서 NA 값을 평균치로 입력하는 방법을 생각한다.

2. 이 방법을 하나의 함수로 만든다.

3. apply함수를 이용하여 2.에서 만든 함수를 전체 데이타프레임에 적용한다. 
